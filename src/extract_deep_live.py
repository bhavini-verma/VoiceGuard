...
def get_audio_files(data_dir):
    audio_files = []
    target_dir = os.path.join(data_dir, '1_genuine', 'sampled_for_training')
    for ext in ['*.wav', '*.flac', '*.mp3', '*.ogg']:
        for file in glob.glob(os.path.join(target_dir, '**', ext), recursive=True):
            audio_files.append((file, 0)) # label is 0 for genuine
    print(f"Found {len(audio_files)} audio files to process from {target_dir}")
    return audio_files
def main():
    ...
    output_csv = os.path.join(features_dir, 'deep_features_live.csv')
